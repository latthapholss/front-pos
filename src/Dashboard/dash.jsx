...
const Dashboard = () => {
    ...
    const calculateTotalSalesForDateRange = (start, end) => {
        ...
        const totalSalesorder = salesInRange.reduce((acc, curr) => acc + curr.totalOrders, 0);

        // Update state with the calculated total sales for the selected range
        setTotalSalesForRange(totalSales);
        setTotalSalesForRange(totalSalesorder);
    ...
    if (selectedSalesData) {
      totalOrders = selectedSalesData.totalOrders; // From selected sales data
    } else if (timeframe === 'weekly') {
      totalOrders = yearlySales.reduce((total, entry) => total + entry.totalSalesForRangeorder, 0); // Sum up for yearly sales if weekly timeframe selected
    } else {
      // Return null or some default component if no conditions met
      return null;
    }
    return (
        <div>
