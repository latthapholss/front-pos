...
import { GETORDERDETAIL, PROMOTIONSTATUS, get, getorderproduct, ip, refundproduct, } from '../Static/api';
...
function SalesDetailPage({ person }) {
  ...
  const [viewReceiptRefundOrderId, setViewReceiptRefundOrderId] = useState(null);
  ...
  const handleReturnProduct = async (opid, order_id) => {
    try {
      ...
      const result = await fetch(`${ip}${refundproduct}`, { // ใช้ `${ip}${refundproduct}` เพื่อรวม URL
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        // แสดง Snackbar เมื่อคืนสินค้าสำเร็จ
        setSnackbarSeverity('success');
        setSnackbarMessage('คืนสินค้าเรียบร้อย');
        setSnackbarOpen(true);

        // โหลดข้อมูลรายการสินค้าใหม่หลังจากคืนสินค้าสำเร็จ
        handleViewOrderProducts(order_id);
      } else {
        // แสดง Snackbar เมื่อเกิดข้อผิดพลาดในการคืนสินค้า
    ...
    try {
      ...
      if (res.success) {
        if (Array.isArray(res.result)) {
          ...
          const uniqueOrders = [...new Map(res.result.map(item => [item.order_id, item])).values()];
          const modifiedData = uniqueOrders.map((item) => ({
            ...
            opid: item.opid,
            product_name: item.product_name, // เปลี่ยนจากรหัสสินค้าเป็นชื่อสินค้า
            itemset_id: item.itemset_id,
            op_status: item.op_status,
            quantity: item.quantity,
            cost_price: item.cost_price,
            unit_price: item.unit_price,
            order_product_date:item.order_product_date,
            order_code: generateOrderCode(item.order_id, 6),
          }));
  ...
  const [orderByField, setOrderByField] = useState('order_date', 'order_code', 'unit_price', 'quantity', 'total_amount', 'op_status');
    ...
    [
      (item) => {
        if (orderByField === 'order_code') {
          ...
          const totalAmountValue = typeof item.total_amount === 'string'
            ? parseFloat(item.total_amount.replace(/[^\d.]/g, '')) || 0
            : item.total_amount || 0;

          return isNaN(totalAmountValue) ? 0 : totalAmountValue;
        } else if (orderByField === 'op_status') {
          return item.op_status === 'คืนสินค้า' ? 0 : item.op_status === 'ขายสินค้าสำเร็จ' ? 0 : 0;
        } else {
  ...
  return (
    <div>
              ...
              <Table sx={{ borderRadius: 2 }}>
                <TableHead style={{ backgroundColor: "#009ae1" }}>
                  <TableRow style={{ color: 'white' }}>
                    <TableCell style={{ color: 'white' }} onClick={() => handleSort('order_code')}>
                      รหัสออเดอร์ {orderByField === 'order_code' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                    </TableCell>
                    <TableCell style={{ color: 'white' }} onClick={() => handleSort('order_date')}>
                      วันที่ออเดอร์ {orderByField === 'order_date' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                    </TableCell>
                    <TableCell style={{ color: 'white' }} onClick={() => handleSort('unit_price')}>
                      ราคาขาย {orderByField === 'unit_price' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                    </TableCell>
                    <TableCell style={{ color: 'white' }} onClick={() => handleSort('quantity')}>
                      จำนวน {orderByField === 'quantity' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                    </TableCell>
                    <TableCell style={{ color: 'white' }} onClick={() => handleSort('total_amount')}>
                      ยอดรวม {orderByField === 'total_amount' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                    </TableCell>

                    <TableCell style={{ color: 'white' }}>รายการสินค้า</TableCell>
                    <TableCell style={{ color: 'white' }}>ใบเสร็จ</TableCell>
                    <TableCell style={{ color: 'white' }}>ใบเสร็จคืนสินค้า</TableCell>
                  ...
                  {sortedItems.map((item, index) => (
                    <TableRow key={item.order_id}>
                      ...
                      <TableCell>
                        ...
                        <Button onClick={() => handleViewOrderProducts(item.order_id)} variant="contained" color="primary">
                          รายการสินค้า
                        </Button>
                      ...
                      <TableCell>
                        ...
                        <Button onClick={() => handleViewReceipt(item.order_id)} variant="contained" style={{ backgroundColor: '#F06B6B', color: 'white' }}>
                          ใบเสร็จสินค้า
                        </Button>
                      ...
                      <TableCell>
                        <Button
                          ...
                          variant="contained"
                          style={{ backgroundColor: '#c8a078', color: 'white' }}
                        >
                          ใบเสร็จคืนสินค้า
                        </Button>
                  ...
                  <TableRow>
                    <TableCell colSpan={13}>
                      ...
                      >
                        ...
                        <Typography variant="caption" sx={{ marginRight: 2 }}>
                          {`${indexOfFirstItem + 1}–${Math.min(indexOfLastItem, ORDERS.length)} of ${ORDERS.length}`}
                        </Typography>
                        <Pagination
      ...
      <Dialog open={viewOrderProductsDialogOpen} onClose={() => setViewOrderProductsDialogOpen(false)}>
        <DialogTitle>รายการสินค้า</DialogTitle>
        <DialogContent>
          {Array.isArray(selectedOrderProductData) && selectedOrderProductData.length > 0 ? (
            <Table>
              <TableHead style={{ backgroundColor: "#009ae1" }}>
                <TableRow>
                  <TableCell style={{ color: 'white' }}>รหัสออเดอร์</TableCell> {/* เพิ่มคอลัมน์รหัสออเดอร์ */}
                  ...
                  <TableCell style={{ color: 'white' }} >
                    สถานะออเดอร์ {orderByField === 'op_status' && <span>{order === 'asc' ? '▲' : '▼'}</span>}
                  </TableCell>
              ...
              <TableBody>
                {selectedOrderProductData.map((product) => (
                  <TableRow key={product.product_id}>
                    ...
                    <TableCell
                      style={{
                        color: product.status === 'success' ? '#44C8A7' : product.status === 'refund' ? '#ffbb00' : 'white',
                        border: '1px solid #e0e0e0',
                        padding: '8px',
                        backgroundColor: product.status === 'success' ? '#E8F8F4' : product.status === 'refund' ? '#fff3d1' : 'white',
                        borderRadius: '5px',
                        textAlign: 'center',
                        fontWeight: 'bold',
                      }}
                    >
                      {product.status === 'success' ? 'ขายสินค้าสำเร็จ' : product.status === 'refund' ? 'คืนสินค้า' : product.status}
                    </TableCell>
                    <TableCell>
                      {product.status === 'success' && (
                        <Button onClick={() => handleReturnProduct(product.opid, product.order_id)} variant="contained" color="primary">
                          คืนสินค้า
                        </Button>
      ...
      {viewReceiptOrderId && (
        <ReceiptDialog open={true} handleClose={() => setViewReceiptOrderId(null)} orderId={viewReceiptOrderId} />
      )}
      {viewReceiptRefundOrderId && (
        <ReceiptRefundDialog open={true} handleClose={() => setViewReceiptRefundOrderId(null)} orderId={viewReceiptRefundOrderId} />
      )}
      <Snackbar
        open={snackbarOpen}
