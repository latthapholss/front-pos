...
const CashierPageDialog = ({
  ...
  const selectedPromotionData = promotions.find(
    (promotionData) => promotionData.id === selectedPromotionId
  );
  useEffect(() => {
    handleGetPromotion();
    handleGetMembers();
  ...
  const calculateFinalTotal = () => {
    ...
    const discountAmount = selectedPromotionData
      ? Helper.discount(totalAmount, selectedPromotionData.discount)
      : 0;
    ...
    if (
      groupedProducts.some((product) => product.category === "กระจกแยกชิ้น")
    ) {
      finalTotal += parseInt(calculateTotalAmountWithGlassCost());
    }
  ...
  const calculateChange = () => {
    ...
    if (
      ...
      change =
        receivedAmount -
        (parseInt(calculateTotalAmountWithAluminumCost()) +
          parseInt(calculateTotalAmountWithGlassCost()) -
          discount -
          pointuse / 10);
    } else if (
      groupedProducts.some((product) => product.category === "กระจก")
  ...
  if (selectedPromotionData && selectedPromotionData.discount) {
    dataToSend.discountedTotalAmount =
      totalAmount -
      Helper.discount(totalAmount, selectedPromotionData.discount);
    dataToSend.discountAmount = Helper.discount(
      totalAmount,
      selectedPromotionData.discount
    );
  }
  ...
  const handleConfirmPayment = async (productIds, productId) => {
    ...
    const calculateAluminumPrice = (product, newQuantity) => {
      if (product.category !== "อะลูมิเนียม") return;
    ...
    const requestData = {
      ...
      selectedProducts: groupedProducts.map((product) => {
        ...
        if (product && product.category === "กระจก") {
          ...
          calculatedCost = calculateGlassPrice(product, newQuantity);
          itemset_id = product.itemset_id;
        } else if (product && product.category === "อะลูมิเนียม") {
    ...
    if (selectedPromotionData && selectedPromotionData.discount) {
      requestData.discountedTotalAmount =
        totalAmount -
        Helper.discount(totalAmount, selectedPromotionData.discount);
      requestData.discountAmount = Helper.discount(
        totalAmount,
        selectedPromotionData.discount
      );
    }
  ...
  const handleGetPromotion = async () => {
    try {
      ...
      if (res.success) {
        ...
        const activePromotions = res.result.filter(
  ...
  const calculateGlassPrice = (productId, newQuantity) => {
    ...
    if (category === "กระจก" || category === "กระจกแยกชิ้น") {
      ...
      const cut = glassDimensions[productId]?.cut || 0;
    ...
    return 0; // Return 0 if not glass category
  };
  ...
  const calculateAluminumPrice2 = (productId, newQuantity) => {
    ...
    if (product && product.category === "อะลูมิเนียม") {
      const cost = (product.price + 100) / 6;
      const dimensions =
        parseInt(product.price) -
        product.price * console.log("ชือ่:35453", dimensions);
      console.log("ราคา:");
      console.log("เมตร:");
  ...
  const calculateTotalAmountWithGlassCost = () => {
    ...
    const totalAmountWithGlassCost = groupedProducts.reduce((acc, product) => {
      if (product.category === "กระจก" || product.category === "กระจกแยกชิ้น") {
        ...
        const glassCost = calculateGlassPrice(
          product.id,
        ...
        return (
          acc +
          product.price * (editedQuantities[product.id] || product.quantity)
        );
      }
    }, 0);
  ...
  const calculateTotalAmountWithAluminumCost = () => {
    ...
    const totalAmountWithAluminumCost = groupedProducts.reduce(
      (acc, product) => {
        if (product.category === "อะลูมิเนียม") {
          // Include the aluminum cost in the total amount

          const aluminumCost = calculateAluminumPrice(
            product.id,
            editedQuantities[product.id] || product.quantity
          );
          ...
          return (
            acc +
            aluminumCost * (editedQuantities[product.id] || product.quantity)
          );
        } else if (
  ...
  const finalTotalAmount =
    groupedProducts.reduce((acc, product) => {
      if (product.category === "กระจก" || product.category === "กระจกแยกชิ้น") {
  ...
  const totalAmountText = () => {
    if (
      ...
      return `${
        (calculateTotalAmountWithGlassCost() || 0) - pointuse / 10
      } บาท ${pointuse && `(ลด ${pointuse / 10} บาท)`} บาท`;
    } else if (
      groupedProducts.some((product) => product.category === "อะลูมิเนียม")
    ) {
      return `${
        (calculateTotalAmountWithAluminumCost() || 0) - pointuse / 10
      } บาท ${pointuse && `(ลด ${pointuse / 10} บาท)`} บาท`;
    } else {
      return `${finalTotalAmount} บาท ${
        pointuse && `(ลด ${pointuse / 10} บาท)`
      }`;
    }
  };
          ...
          >
            <TableHead>
              <TableRow>
                ...
                <TableCell
                  align="right"
                  sx={{ padding: "6px", textAlign: "center", width: "100px" }}
                >
                  ลบ
                </TableCell>
              </TableRow>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">
                      <TextField
                        ...
                        onChange={(e) =>
                          handleEditQuantity(product.id, e.target.value)
                        }
                      />
                    </TableCell>
                    <TableCell align="right">
                      {["กระจก", "อะลูมิเนียม", "กระจกแยกชิ้น"].includes(
                        product.category
                      )
                        ? (product.category === "กระจก" ||
                          product.category === "กระจกแยกชิ้น"
    ...
    : (product.price * (editedQuantities[product.id] || product.quantity)).toLocaleString()
                        ...
                        <TableBody>
                          <TableCell align="">
                            <TextField
                              ...
                              label="ความยาว (เมตร)"
                              value={
                                aluminumDimensions[product.id]?.width2 || ""
                              }
                              onChange={(e) => {
                                const value = e.target.value;
                                if (value > product.product_length) {
                                  alert(
                                    `ความยาวไม่สามารถเกิน ${product.product_length} เมตร`
                                  );
                                  setAluminumDimensions((prevDimensions) => ({
                                    ...prevDimensions,
                                    [product.id]: {
                                      ...prevDimensions[product.id],
                                      width2: "",
                                    },
                                  }));
                                } else if (value > 0) {
                                  ...
                                  alert("กรุณาป้อนค่าที่มากกว่าหรือเท่ากับ 0");
                                }
                              }}
                              sx={{ width: "175px" }}
                            />
                            ...
                            <TextField
                              ...
                              onChange={(e) => {
                                ...
                                if (
                                  ...
                                  alert(
                                    "ค่าต้องมากกว่าหรือเท่ากับ 1 และไม่สามารถเป็นค่าลบ"
                                  );
                                }
                              }}
                              sx={{ width: "175px" }}
                        ...
                        <TableBody>
                          <TableCell align="">
                            <TextField
                              ...
                              onChange={(e) => {
                                const value = e.target.value;
                                const maxLength = product.product_length;
                                if (value > maxLength) {
                                  alert(
                                    `ความยาวไม่สามารถเกิน ${maxLength} นิ้ว`
                                  );
                                  setGlassDimensions((prevDimensions) => ({
                                    ...prevDimensions,
                                    [product.id]: {
                          ...
                          <TableCell align="">
                            <TextField
                              type="number"
                              label="ความกว้าง (นิ้ว)"
                              value={glassDimensions[product.id]?.width || ""}
                              onChange={(e) => {
                                const value = e.target.value;
                                const maxHeight = product.product_width;
                                if (value > maxHeight) {
                                  alert(
                                    `ความกว้างไม่สามารถเกิน ${maxHeight} นิ้ว`
                                  );
                                  setGlassDimensions((prevDimensions) => ({
                                    ...prevDimensions,
                                    [product.id]: {
              ...
              <TableRow>
                ...
                <TableCell align="right">
                  ...
                  groupedProducts.some(
                    (product) => product.category === "อะลูมิเนียม"
                  ) ? (
                    `${(
                      parseInt(calculateTotalAmountWithAluminumCost()) -
                      pointuse / 10 +
                    ...
                    `${(
                      (calculateTotalAmountWithGlassCost() || 0) -
                      pointuse / 10
                    ).toLocaleString()} บาท ${
                      pointuse && `(ลด ${pointuse / 10} บาท)`
                    } `
                    ...
                    `${(
                      (calculateTotalAmountWithAluminumCost() || 0) -
                      pointuse / 10
                    ).toLocaleString()} บาท ${
                      pointuse && `(ลด ${pointuse / 10} บาท)`
                    } `
                    ...
                    >
                      {totalAmount.toLocaleString()} บาท <br />
              ...
              <TableRow>
                ...
                <TableCell align="right">
                  ...
                  groupedProducts.some(
                    ...
                    <React.Fragment>
                      {`${(
                        calculateTotalAmountWithGlassCost() -
                        pointuse / 10 -
                        (selectedPromotionData
                          ? Helper.discount(
                              totalAmountWithGlassCost,
        ...
        >
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="เลือกสมาชิก" />
            )}
          />
        ...
        {selectedMember && (
          ...
          >
            <Typography sx={{ mb: 2 }}>
              คะแนนของสมาชิก = {selectedMember.point}
            </Typography>
            <TextField
              value={pointuse}
